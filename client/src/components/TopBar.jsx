...
export default function TopBar({ gameState, tableId, isTraining, onOpenSidebar, onAddAI }) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2 bg-gray-900/80 border-b border-gray-800 z-40 relative">
      <button
        onClick={() => navigate('/')}
        className="text-gray-400 active:text-white transition-colors p-1"
      >
      ...
      <div className="text-center flex-1">
        <div className="text-xs text-gray-500">
          ...
          <button
            ...
            className="ml-2 text-gold/70 active:text-gold inline-flex items-center gap-0.5"
          >
      ...
      {showMenu && (
        <>
          ...
          <div className="absolute right-2 top-full mt-1 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl z-50 w-56 overflow-hidden animate-fade-in">
            ...
            <button
              onClick={() => { onOpenSidebar('leaks'); setShowMenu(false); }}
              className="w-full text-left px-4 py-3 text-sm text-gray-200 hover:bg-gray-700 active:bg-gray-600 transition-colors border-b border-gray-700"
            >
              Leak Report
            </button>
            <button
              onClick={() => { onOpenSidebar('history'); setShowMenu(false); }}
              className="w-full text-left px-4 py-3 text-sm text-gray-200 hover:bg-gray-700 active:bg-gray-600 transition-colors border-b border-gray-700"
            >
              Hand History
            </button>
            <div className="border-t border-gray-700">
              ...
              {['gto_bot', 'aggro_pro', 'nit_reg', 'loose_rec', 'elite_hybrid'].map(type => (
                <button
                  key={type}
                  onClick={() => { onAddAI(type); setShowMenu(false); }}
